	...
	up: async (queryInterface, Sequelize) => {
		...
		const [[mentorCountResult], [menteeCountResult]] = await Promise.all([
			queryInterface.sequelize.query(
				'SELECT count(*) AS "count" FROM mentor_extensions WHERE name IS NULL OR email IS NULL;'
			),
			queryInterface.sequelize.query(
				'SELECT count(*) AS "count" FROM user_extensions WHERE name IS NULL OR email IS NULL;'
			),
		])
		...
		if (mentorCount > 0) {
			const [mentors] = await queryInterface.sequelize.query(
				'SELECT user_id FROM mentor_extensions WHERE name IS NULL OR email IS NULL;'
			)
		...
		if (menteeCount > 0) {
			const [mentees] = await queryInterface.sequelize.query(
				'SELECT user_id FROM user_extensions WHERE name IS NULL OR email IS NULL;'
			)
